...
- LLM: `ai` (Vercel AI SDK) + `@openrouter/ai-sdk-provider` + `zod` v4. Only in `server/`, `engine/contracts/` and `engine/calls/`.
- Serverless: Vercel functions bundled from `server/` into `api/` (`OPENROUTER_API_KEY` from env). No DB, no auth, no docker.
...
server/      Vercel function sources: extract.ts (contract text → JSON), analyze-call.ts; tests next to them
api/         generated by `npm run build` (esbuild bundles of server/*.ts, git-ignored) — Vercel deploys these
