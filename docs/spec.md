...
# Forest inventory valuation, contract extraction and call quality — take-home assignment
...
page, README, CLI for task 1, synthetic sample files, tests on two inventory oracles (private real
PDF, public synthetic PDF) and the synthetic call-day oracle.
...
- **Client-side calculation, serverless only for the LLM.** No DB, no auth, no infra. Vercel Hobby
  request limits are respected by sending extracted text for PDFs and bounded MP3 audio for the
  separate transcription endpoint. Original PDFs are never uploaded.
...
- **Language.** UI, report, decisions page, e-mail draft — Latvian. Code, comments, README, commit
  messages — English. Do not add Russian to tracked files. The existing docs/decisions.ru.md is
  an archived planning draft; DECISIONS.md is the maintained LV page source. docs/research and
  docs/task are git-ignored.
...
- **Call scoring stays deterministic.** A model may propose criterion observations for a new call,
  ...
  and flags low-confidence cases for human review. Synthetic calls use committed observations.
...
For a new transcript, `POST /api/analyze-call` may use a model to return grounded observations in
the fixed schema. This endpoint receives reviewed text and start-time context; audio uses the
separate transcription endpoint below. Analysis accepts at most 100 KB of text, rate-limits
best-effort and returns a clear 503 when no key is configured. The model
...
TXT file selection and drag-and-drop share the same reader. Reject unsupported formats, empty and
oversized text files before analysis while preserving any previously entered transcript. The shared picker
also accepts MP3 through the separate transcription flow below. A file drop must not navigate the
browser away from the form.
...
### Task 3 — Current process and proposed pilot

The process panel and manager-facing documents describe manual TXT/MP3 input, transcription review,
source excerpt selection and code-calculated scores as the current prototype. Call/day views update
after an analysis and exist only in page memory until reload; outputs can be downloaded. Role tabs
are demonstration views. Telephony integration, account-based access, review-task assignment and
scheduled delivery are planned work, not running features.

Label the rollout schedule, 30–50-call calibration set and percentage thresholds as proposals to
agree with the process owner. They have not been validated in a representative real-call pilot.
Synthetic live checks verify the implementation path, not those quality or savings targets. The
pilot plan measures transcription word, speaker and important-number errors separately, compares
analysis before/after transcript correction and includes review/correction effort in time savings.
...
- The original Russian decisions draft is retained as an explicitly marked historical document;
  the maintained public decisions page is DECISIONS.md.

## Next Steps

→ `PLAN.md` for implementation and verification status; `docs/NIGHT-REPORT.md` for current handoff
checks. The employer e-mail remains a draft in `docs/email.lv.md`.
...
3. `/ligumi` — dropzone + three owned synthetic PDF downloads and separate cached previews;
   "Izvilkt datus" → progress →
...
5. `/zvani` — TXT/MP3 picker and drop area, audio transcription/review and downloadable samples;
   synthetic sample day, per-call score and grounded rubric table;
